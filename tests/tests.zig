...
//     const original_shards_present: [count]bool = @splat(true);
//     const recovery_shards_present: [count]bool = @splat(false);

//     try encodeDecodeCycle(
//         allocator,
//         count,
//         SHARD_BYTES,
//         input,
...
test "encode" {
    ...
    const parity = try encode(
        allocator,
        &original,
        SHARD_COUNT, // we want the same number of input shards as output
    );
    defer {
        for (parity) |p| allocator.free(p);
        allocator.free(parity);
    }

    const expected: [SHARD_COUNT][SHARD_BYTES]u8 = @import("tests/encode_data.zon");
    for (expected, parity) |e, r| try testing.expectEqual(&e, r);
}
